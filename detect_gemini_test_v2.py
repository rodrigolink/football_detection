...
# Tracker Memory Buffers
TRACKER_MEMORY_BUFFER = 75  
TRACKER_TRACK_THRESH = 0.10  
...
def process_and_export_30s_tracking(video_path):
    ...
    if total_frames <= TRACKING_WINDOW_FRAMES:
        ...
        start_frame = random.randint(0, total_frames - TRACKING_WINDOW_FRAMES - 1)
        actual_processing_frames = TRACKING_WINDOW_FRAMES
        
    tracker = sv.ByteTrack()
    if hasattr(tracker, 'track_thresh'): tracker.track_thresh = TRACKER_TRACK_THRESH
    if hasattr(tracker, 'track_buffer'): tracker.track_buffer = TRACKER_MEMORY_BUFFER
    if hasattr(tracker, 'match_thresh'): tracker.match_thresh = TRACKER_MATCH_THRESH
    ...
    # We calculate the cushion boundary in real-world metric pixel-space.
    # This expands the accepted zone safely outwards for corner kicks and throw-ins.
    ...
    # --------------------------------------------------------------------------
    # PASS 1: TRACK ONCE & RECORD TRAJECTORIES
    # --------------------------------------------------------------------------
    print("[+] Executing Phase 1/2: High-Resolution Frame Track Analytics")
    ...
    for f_idx in range(actual_processing_frames):
        ...
        results = model(big_frame, imgsz=1920, conf=PLAYER_CONF_BASE, verbose=False, iou=PLAYER_IOU_THRESHOLD)[0]
        ...
        for bbox, _, _, _, tracker_id, _ in tracked_players:
            x1, y1, x2, y2 = bbox
            cx, cy = (x1 + x2) / 2.0, y2
            
            # Convert frame pixels directly into metric canvas coordinates
            point = np.array([[[cx, cy]]], dtype=np.float32)
            transformed = cv2.perspectiveTransform(point, H)[0][0]
            canvas_x, canvas_y = transformed[0], transformed[1]
            
            # GEOMETRIC CHECK: Test if feet are inside the PADDED cushion polygon
            is_inside = cv2.pointPolygonTest(padded_polygon, (float(canvas_x), float(canvas_y)), False)
            if is_inside < 0:
                continue # Safely drops spectators/bench while keeping corner takers!
